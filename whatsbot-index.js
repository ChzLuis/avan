...
const LARAVEL_URL  = 'http://127.0.0.1:8080';
const LARAVEL_HOST = 'admin.mercadosmayoristas.com.pe';
const BOT_PORT     = 3001;
const BOT_TYPE     = 'main';
...
async function laravelGet(path) {
    try {
        const res = await fetch(`${LARAVEL_URL}/${path}`, { headers: { Host: LARAVEL_HOST } });
...
async function laravelPost(path, body) {
    try {
        const res = await fetch(`${LARAVEL_URL}/${path}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', Host: LARAVEL_HOST },
...
async function loadFlow(phone = '') {
    ...
    try {
        ...
        const res = await fetch(`${LARAVEL_URL}/wa/flow-config?${params}`, { headers: { Host: LARAVEL_HOST } });
        if (res.ok) {
...
client.on('ready', async () => {
    ...
    try {
        ...
        const res = await fetch(`${LARAVEL_URL}/wa/config?token=${BOT_TOKEN}&phone=${myNumber}`, { headers: { Host: LARAVEL_HOST } });
        const data = await res.json();
        if (data.ok) {
