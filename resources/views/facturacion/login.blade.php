...
<head>
    ...
    <title>{{ $project->name }} — Portal Facturación</title>
...
@php
    $bg1 = $project->setting('login_color1', '#1d4ed8');
    $bg2 = $project->setting('login_color2', '#1e40af');
    $bgStyle = "background:linear-gradient(135deg, {$bg1} 0%, {$bg2} 100%);";
@endphp
    ...
    <div class="hidden lg:flex lg:w-1/2 xl:w-3/5 relative overflow-hidden" style="{{ $bgStyle }}">
        ...
        <div class="relative z-10 flex flex-col justify-center px-16 text-white">
            <div class="flex items-center gap-3 mb-12">
                ...
                <div>
                    <p class="text-xs text-white/60 font-medium uppercase tracking-wider">Portal Facturación</p>
                    <p class="text-xl font-bold">{{ $project->name }}</p>
                </div>
            </div>

            <h1 class="text-4xl xl:text-5xl font-bold leading-tight mb-4">
                Gestión de ventas<br>y documentos
            </h1>
            <p class="text-lg text-white/70 max-w-sm leading-relaxed">
                POS, pedidos, cotizaciones y facturación electrónica en un solo lugar.
            </p>
            ...
            <div class="mt-12 space-y-4">
                @foreach([
                    ...
                    ['icon'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'text'=>'Preparado para SUNAT'],
                ] as $feat)
    ...
    <div class="w-full lg:w-1/2 xl:w-2/5 flex items-center justify-center bg-white px-6 py-12">
        <div class="w-full max-w-md">
            ...
            <div class="flex items-center gap-2 mb-8 lg:hidden">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:{{ $bg1 }}">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/>
                    </svg>
                </div>
                <span class="text-lg font-bold text-gray-900">{{ $project->name }}</span>
            </div>

            <div class="mb-8">
                <p class="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-1">Portal Facturación</p>
            ...
            <form method="POST" action="{{ route('facturacion.login.post', $project->slug) }}"
                ...
                <button type="submit" :disabled="loading"
                        class="w-full py-3 px-4 text-white font-semibold rounded-xl text-sm transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-sm"
            ...
            <p class="mt-5 text-center text-xs">
                <a href="{{ route('portal.password.request', 'facturacion') }}" class="text-gray-400 hover:text-gray-600 transition">
