@php
    $storeUrl  = route('facturacion.cotizaciones.store', $project->slug);
    $rucUrl    = route('facturacion.ruc.lookup', $project->slug);
    $listUrl   = route('facturacion.cotizaciones', $project->slug);
@endphp

<x-facturacion-layout :project="$project" pageTitle="Nueva Cotización">
...
<div class="flex-1 overflow-y-auto bg-gray-100"
    ...
    <div class="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-20 shadow-sm">
        <div class="flex items-center gap-3">
            <a href="{{ $listUrl }}" class="text-gray-400 hover:text-gray-600 p-1 rounded-lg hover:bg-gray-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
            <span class="text-sm font-bold text-gray-800">Nueva Cotización</span>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ $listUrl }}" class="px-4 py-2 text-sm text-gray-600 font-medium rounded-xl border border-gray-200 hover:bg-gray-50">
                Cancelar
            </a>
            <button @click="guardar()" :disabled="saving"
                    class="px-6 py-2 text-sm font-bold rounded-xl text-white bg-violet-600 hover:bg-violet-700 disabled:opacity-60 flex items-center gap-2">
                <svg x-show="saving" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                </svg>
                <span x-text="saving ? 'Guardando...' : 'Guardar cotización'"></span>
            </button>
        </div>
    </div>
    ...
    <div class="p-6 space-y-4 max-w-6xl mx-auto">
            ...
            <div class="p-6 grid grid-cols-3 gap-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="form.client_address" placeholder="Dirección"
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
                    </div>
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-[10px] font-semibold text-gray-400 uppercase mb-1">Teléfono</label>
                            <input type="text" x-model="form.client_phone" placeholder="999 000 000"
                                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
                        </div>
                        <div>
                            <label class="block text-[10px] font-semibold text-gray-400 uppercase mb-1">Email</label>
                            <input type="email" x-model="form.client_email" placeholder="cliente@email.com"
                                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div>
                ...
                <div class="grid items-center border-b border-gray-100 transition-colors"
                    ...
                    <input type="number" x-model="line.qty" min="1" step="1"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <textarea x-model="form.notes" rows="3"
                      placeholder="Condiciones, notas adicionales, vigencia..."
        ...
        <div class="flex justify-end gap-3 pb-6">
            <a href="{{ $listUrl }}" class="px-5 py-2.5 text-sm text-gray-600 font-medium rounded-xl border border-gray-200 hover:bg-gray-50">
                Cancelar
            </a>
            <button @click="guardar()" :disabled="saving"
...
function cotizPage() {
    ...
    return {
        ...
        form: {
            ...
            valid_until: '', payment_method: '', payment_condition: 'Contado', notes: '',
            lines: [{ desc:'', unit:'NIU', qty:1, price:0, showSugg:false, suggestions:[] }],
        },
        init() {},
        ...
        async guardar() {
            ...
            try {
                ...
                const data = await res.json();
                if (!res.ok) {
                    this.errorMsg = data.errors ? Object.values(data.errors).flat().join(' ') : (data.message||'Error.');
                } else {
                    window.location.href = '{{ $listUrl }}';
                }
            } catch(e) { this.errorMsg = 'Error de red.'; }
            this.saving = false;
        },
    };
}
