...
{{-- TOP BAR CATHA --}}
<div style="background:#1a1209;border-bottom:1px solid rgba(201,168,76,.2);" class="text-xs py-2">
...
<header style="background:#ffffff;border-bottom:1px solid rgba(201,168,76,.25);box-shadow:0 2px 20px rgba(0,0,0,.06);" class="sticky top-0 z-30">
        ...
        <div x-show="searchOpen && suggestions.length>0" x-cloak
          ...
          <template x-for="(p,i) in suggestions" :key="p.id">
            <button @click="selectSuggestion(p)" :class="searchIdx===i?'bg-gray-50':''"
                    class="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-amber-50 transition text-left border-b border-gray-100 last:border-0">
              <div class="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 bg-gray-50">
  ...
  <nav style="border-top:1px solid rgba(201,168,76,.2);background:#faf7f0;" class="hidden lg:block">
    <div class="relative">
      {{-- Flecha izquierda --}}
      <button id="nav-prev" onclick="document.getElementById('cat-scroll').scrollBy({left:-200,behavior:'smooth'})"
              class="absolute left-0 top-0 h-full px-3 z-10 flex items-center"
              style="background:linear-gradient(90deg,#faf7f0 60%,transparent);">
      ...
      {{-- Categorias scroll --}}
      <div id="cat-scroll" class="flex items-center overflow-x-auto no-scroll px-10">
        <button @click="filterCat=''; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
                :class="filterCat==='' ? 'text-gold border-b-2 border-gold' : 'text-gray-500 hover:text-gray-800 border-b-2 border-transparent'"
                class="px-5 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap transition flex-shrink-0">Todo</button>
        @foreach($categories as $cat)
        <button @click="filterCat='{{ $cat->id }}'; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
                :class="filterCat==='{{ $cat->id }}' ? 'text-gold border-b-2 border-gold' : 'text-gray-500 hover:text-gray-800 border-b-2 border-transparent'"
                class="px-5 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap transition flex-shrink-0">{{ $cat->name }}</button>
        @endforeach
      </div>
      {{-- Flecha derecha --}}
      <button id="nav-next" onclick="document.getElementById('cat-scroll').scrollBy({left:200,behavior:'smooth'})"
              class="absolute right-0 top-0 h-full px-3 z-10 flex items-center"
              style="background:linear-gradient(270deg,#faf7f0 60%,transparent);">
  ...
  <div class="lg:hidden flex gap-1 px-4 pb-2 pt-1 no-scroll overflow-x-auto" style="border-top:1px solid rgba(201,168,76,.2);background:#faf7f0;">
    <button @click="filterCat=''; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
            :class="filterCat===''?'bg-gold text-gray-900':'bg-gray-50 text-gray-600'"
            class="flex-shrink-0 text-xs font-semibold px-3 py-1.5 rounded-full transition">Todo</button>
    @foreach($categories as $cat)
    <button @click="filterCat='{{ $cat->id }}'; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
            :class="filterCat==='{{ $cat->id }}'?'bg-gold text-gray-900':'bg-gray-50 text-gray-600'"
...
{{-- ═══ HERO ═══ --}}
<section class="relative overflow-hidden flex items-end" style="min-height:520px;background:{{ $heroBg }};">
  {{-- Fondo decorativo --}}
  <div class="absolute inset-0" style="background:linear-gradient(135deg,#fdf8f0 0%,#f5ead8 50%,#fdf8f0 100%);"></div>
  <div class="absolute inset-0 opacity-[0.08]" style="background-image:repeating-linear-gradient(45deg,#b8973a 0,#b8973a 1px,transparent 0,transparent 50%);background-size:40px 40px;"></div>
  {{-- Círculo decorativo dorado --}}
  <div class="absolute right-0 top-0 w-[600px] h-[600px] rounded-full opacity-20" style="background:radial-gradient(circle,var(--gold) 0%,transparent 70%);transform:translate(30%,-30%);"></div>
  {{-- Texto hero --}}
  <div class="relative z-10 max-w-[1400px] mx-auto px-6 pb-16 pt-20 w-full">
    <div class="max-w-2xl">
      @if($heroBadge)
      <div class="inline-flex items-center gap-2 mb-5 border border-gold/60 px-4 py-1.5 rounded-full bg-white/60">
      ...
      <p class="text-gray-700 text-lg mb-8 leading-relaxed max-w-lg">{{ $heroSub }}</p>
      <div class="flex flex-wrap gap-3">
        <button @click="document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
                class="btn-gold px-7 py-3 rounded-full text-sm font-bold uppercase tracking-wide transition">
  ...
  {{-- Divisor ondulado --}}
  <div class="absolute bottom-0 left-0 right-0" style="height:40px;background:var(--navy);clip-path:ellipse(55% 100% at 50% 100%);"></div>
</section>
  ...
  <div class="max-w-[1400px] mx-auto px-6">
    <div class="flex gap-4 no-scroll overflow-x-auto pb-1">
      ...
      <button @click="filterCat=''"
        ...
        <span :class="filterCat===''?'text-gold':'text-gray-600'" class="text-xs font-semibold">Todo</span>
      </button>
      ...
      <button @click="filterCat='{{ $cat->id }}'; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
        ...
        <span :class="filterCat==='{{ $cat->id }}'?'text-gold':'text-gray-600'" class="text-xs font-semibold text-center leading-tight">{{ $cat->name }}</span>
      </button>
      @endforeach
    </div>
  </div>
...
<section class="py-12 max-w-[1400px] mx-auto px-6">
  <div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-3">
      ...
      <h2 class="serif font-bold text-2xl text-gray-800">Precios especiales de hoy</h2>
    </div>
    <button @click="onSaleFilter=true; document.getElementById('catalogo').scrollIntoView({behavior:'smooth'})"
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
    ...
    <article class="lic-card group cursor-pointer rounded-2xl overflow-hidden" style="background:#fff;border:1px solid rgba(255,255,255,.06);"
             id="oferta-{{ $p->id }}">
      ...
      <div class="p-3">
        <a href="{{ route('public.product', [$project->slug, $p->id]) }}" class="text-gray-700 text-sm font-semibold line-clamp-2 leading-snug hover:text-gold transition block mb-2">{{ $p->name }}</a>
        @if(!$isQuoteOnly || $quotePriceDisp==='show')
        <div class="flex items-baseline gap-2">
          <span class="text-gold font-black text-lg">{{ $currency }} {{ number_format($p->price,2) }}</span>
          <span class="text-gray-500 text-xs line-through">{{ $currency }} {{ number_format($p->compare_price,2) }}</span>
        </div>
        @endif
      </div>
    </article>
...
{{-- ═══ BUSCADOR MÓVIL ═══ --}}
<div class="md:hidden px-4 py-2 sticky top-[105px] z-10 border-b border-gray-100" style="background:#fff;">
...
<section id="catalogo" class="max-w-[1400px] mx-auto px-6 pb-24 pt-8">
  ...
  <div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-3">
      ...
      <h2 class="serif font-bold text-2xl text-gray-800">
    ...
    <div class="flex items-center gap-2">
      <button @click="filterOpen=true"
              class="flex items-center gap-1.5 text-xs font-semibold border border-gray-200 rounded-lg px-3 py-1.5 hover:border-gold/40 transition relative text-gray-600">
      ...
      <select x-model="sortBy" class="text-xs border border-gray-200 px-2 py-1.5 rounded-lg outline-none text-gray-600 cursor-pointer hover:border-gold/40 transition" style="background:#fff;">
    ...
    <aside class="w-[180px] flex-shrink-0 hidden xl:block space-y-6">
      <div>
        ...
        <div class="space-y-0.5">
          <button @click="filterCat=''"
                  :class="filterCat===''?'text-gold font-bold':'text-gray-500 hover:text-gray-500'"
                  class="block w-full text-left text-sm py-1.5 transition">
            Todo el catálogo
          </button>
          @foreach($categories as $cat)
          <button @click="filterCat='{{ $cat->id }}'"
                  :class="filterCat==='{{ $cat->id }}'?'text-gold font-bold':'text-gray-500 hover:text-gray-500'"
                  class="block w-full text-left text-sm py-1.5 transition flex items-center justify-between">
            <span>{{ $cat->name }}</span>
            <span class="text-xs text-gray-700">{{ $cat->products->count() }}</span>
          </button>
          @endforeach
        </div>
      </div>
      ...
      <div>
        ...
        <div class="space-y-2">
          ...
          <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer hover:text-gray-500 transition">
    ...
    <div class="flex-1">
      ...
      <div x-show="filterCat==='' || filterCat==='{{ $cat->id }}'" class="mb-12">
        <div class="flex items-center gap-3 mb-5">
          <h3 class="serif font-bold text-gray-800 text-lg">{{ $cat->name }}</h3>
          <div class="flex-1 border-t border-gray-100"></div>
          <span class="text-xs text-gray-600">{{ $catAllProducts->count() }} productos</span>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-4" data-products-grid>
          ...
          <article class="lic-card group cursor-pointer rounded-2xl overflow-hidden"
                   style="background:#fff;border:1px solid rgba(255,255,255,.06);"
                   id="producto-{{ $p->id }}"
            ...
            <a href="{{ route('public.product', [$project->slug, $p->id]) }}" class="lic-img block relative overflow-hidden" style="aspect-ratio:1/1;">
              @if($p->mainImage)
              <img src="{{ $p->main_image_url }}" alt="{{ $p->name }}" loading="lazy" class="w-full h-full object-contain p-4" style="background:rgba(255,255,255,.04);">
              @else
              <div class="w-full h-full flex items-center justify-center" style="background:rgba(255,255,255,.03);">
            ...
            <div class="p-3">
              ...
              <a href="{{ route('public.product', [$project->slug, $p->id]) }}"
                 class="text-gray-700 text-sm font-semibold line-clamp-2 leading-snug hover:text-gold transition block mb-2">{{ $p->name }}</a>
              ...
              <div class="flex items-baseline gap-2">
                ...
                <span class="text-gray-600 text-xs line-through">{{ $currency }} {{ number_format($p->compare_price,2) }}</span>
                @endif
              </div>
              @else
              <p class="text-xs text-gray-600 italic">Precio a consultar</p>
              @endif
            </div>
          </article>
          @endforeach
      ...
      <div x-show="noResults" class="text-center py-20">
        ...
        <p class="serif font-bold text-gray-800 text-xl mb-2">Sin resultados</p>
...
{{-- FOOTER CATHA --}}
<footer style="background:#1a1209;border-top:1px solid rgba(201,168,76,.2);">
  <div style="height:2px;background:linear-gradient(90deg,transparent,var(--gold),#e8c96a,var(--gold),transparent);"></div>
  <div class="max-w-[1400px] mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-3 gap-12">
    ...
    <div class="flex flex-col items-start gap-4">
      ...
      <p class="text-gray-400 text-sm leading-relaxed max-w-xs">{{ $footerTagline }}</p>
...
<div x-show="filterOpen" x-cloak class="xl:hidden fixed inset-0 z-50 flex flex-col justify-end">
  ...
  <div class="relative rounded-t-3xl shadow-2xl max-h-[80vh] flex flex-col"
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
      <h3 class="font-black text-gray-800">Filtros</h3>
      <button @click="filterOpen=false" class="p-1.5 rounded-lg hover:bg-gray-100 transition">
        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
      </button>
    </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <button @click="priceFilter='{{ $val }}'"
                  :class="priceFilter==='{{ $val }}' ? 'border-gold text-gold font-bold' : 'border-gray-200 text-gray-600'"
                  class="border rounded-xl px-3 py-2 text-sm text-left transition {{ $val==='' ? '' : '' }}">{{ $label }}</button>
          @endforeach
        </div>
      </div>
      <div>
        <label class="flex items-center gap-3 p-3 rounded-xl cursor-pointer border border-gray-200 transition"
               :class="onSaleFilter?'border-gold bg-gold/10':''">
          <input type="checkbox" x-model="onSaleFilter" class="w-4 h-4 rounded" style="accent-color:var(--gold)">
          <span class="text-sm font-medium text-gray-500">Solo productos en oferta</span>
        </label>
      </div>
      <div>
        ...
        <div class="space-y-2">
          ...
          <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer transition"
                 :class="sortBy==='{{ $val }}'?'border-gold bg-gold/10':''">
            <input type="radio" x-model="sortBy" value="{{ $val }}" class="w-4 h-4" style="accent-color:var(--gold)">
            <span class="text-sm font-medium text-gray-500">{{ $lbl }}</span>
          </label>
          @endforeach
        </div>
      </div>
    ...
    <div class="flex gap-3 px-5 py-4 border-t border-gray-100 flex-shrink-0">
      <button @click="priceFilter='';priceMin=0;priceMax=0;onSaleFilter=false;sortBy='default'"
              class="flex-1 py-3 rounded-xl text-sm font-bold border border-gray-200 text-gray-600 hover:bg-amber-50 transition">
        Limpiar
      </button>
      ...
      <div x-show="!orderSent" class="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        ...
        <div>
          ...
          <div x-show="couponApplied" class="flex items-center justify-between bg-green-50 border border-green-200 px-4 py-2.5 text-sm rounded-xl">
            ...
            <button @click="removeCoupon" class="text-gray-500 hover:text-red-500 ml-3 text-lg">&times;</button>
          </div>
      ...
      <div x-show="!orderSent" class="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        ...
        <div class="space-y-2">
          ...
          <button @click="openMercadoPago()" :disabled="payLoading"
            ...
            <svg x-show="!payLoading" class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
