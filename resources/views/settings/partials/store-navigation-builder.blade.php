@php
    ...
    $destinationLabels = [
        ...
        'subcategory'=>'Subcategoría','about'=>'Nosotros','contact'=>'Contacto','blog'=>'Blog',
...
<section id="constructor-navegacion" class="constructor-panel constructor-panel--navegacion store-navigation-builder" aria-labelledby="navigation-builder-title">
    ...
    <form method="POST" action="{{ route('settings.storefront.menu.items.store') }}" class="snb-card" x-data="{type:'home'}">
        @csrf
        <div class="snb-card-title"><div><h3>Agregar opción</h3><p>El texto visible siempre puede modificarse.</p></div></div>
        <div class="snb-grid snb-grid-3">
            <label><span>Texto visible</span><input required name="label" maxlength="100" placeholder="Ej. Compra online"></label>
            <label><span>Destino</span><select name="destination_type" x-model="type">@foreach($destinationLabels as $key=>$label)<option value="{{ $key }}">{{ $label }}</option>@endforeach</select></label>
            <label><span>Opción principal</span><select name="parent_id"><option value="">Sin superior</option>@foreach($roots as $root)<option value="{{ $root->id }}">{{ $root->label }}</option>@endforeach</select><small>Déjalo vacío para crear una opción principal.</small></label>
            <label x-show="['category','subcategory','page'].includes(type)"><span>Categoría o página</span><select name="destination_id" :disabled="!['category','subcategory','page'].includes(type)"><option value="">Selecciona</option><optgroup label="Categorías">@foreach($menuCategories->whereNull('parent_id') as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</optgroup><optgroup label="Subcategorías">@foreach($menuCategories->whereNotNull('parent_id') as $category)<option value="{{ $category->id }}">{{ $category->parent?->name }} / {{ $category->name }}</option>@endforeach</optgroup><optgroup label="Páginas personalizadas">@foreach($pages as $page)<option value="{{ $page->id }}">{{ $page->title }}</option>@endforeach</optgroup></select></label>
            <label x-show="type==='external'"><span>URL externa</span><input type="url" name="url" :disabled="type!=='external'" placeholder="https://ejemplo.com"><small>Solo enlaces http o https.</small></label>
            <label><span>Abrir enlace</span><select name="target"><option value="_self">En la misma pestaña</option><option value="_blank">En otra pestaña</option></select></label>
        </div>
