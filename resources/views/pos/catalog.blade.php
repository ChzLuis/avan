...
<section id="catalogo" class="max-w-[1400px] mx-auto px-4 pb-16">
  ...
  <div class="sticky top-16 z-20 bg-white/95 backdrop-blur-sm -mx-4 px-4 py-3 mb-6 border-b border-gray-100 shadow-sm flex items-center justify-between flex-wrap gap-3">
    <div class="flex items-center gap-3">
      ...
      <span x-show="filterCat!=='' || search!=='' || priceFilter!=='' || onSaleFilter || stockFilter"
    ...
    <div class="flex items-center gap-2">
      ...
      <span x-show="filterCat!=='' || search!=='' || priceFilter!=='' || onSaleFilter" class="text-sm text-gray-500">
        <button @click="filterCat=''; search=''; priceFilter=''; onSaleFilter=false; stockFilter=false"
  ...
  <div class="flex gap-6">
    ...
    <aside class="w-[210px] flex-shrink-0 hidden xl:block space-y-3">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-4">
        ...
        <div class="space-y-2">
          ...
          <label class="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
            <input type="checkbox" x-model="stockFilter" class="accent-[var(--c)] rounded">
    ...
    <div class="flex-1">
      ...
      <div x-show="filterCat==='' || filterCat==='{{ $cat->id }}' || filterParent==='{{ $cat->id }}' || [{{ $cat->children->pluck('id')->map(fn($id)=>"'".$id."'")->join(',') }}].includes(filterCat)" class="mb-12">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4" data-products-grid>
          ...
          <article id="producto-{{ $p->id }}"
                   ...
                   x-show="matchProduct('{{ strtolower(addslashes($p->name)) }}', {{ $p->price }}, {{ $p->compare_price ?? 'null' }}, {{ $p->stock ?? 'null' }}) && ({{ $loop->index }} < 8 || expandedCats['{{ $cat->id }}'])"
                   data-price="{{ $p->price }}"
          ...
          <article id="producto-{{ $p->id }}"
                   class="prod-card bg-white rounded-2xl border border-gray-200 overflow-hidden group"
                   x-show="(filterCat==='' || filterCat==='{{ $sub->id }}' || filterParent==='{{ $cat->id }}') && matchProduct('{{ strtolower(addslashes($p->name)) }}', {{ $p->price }}, {{ $p->compare_price ?? 'null' }}, {{ $p->stock ?? 'null' }})"
                   data-price="{{ $p->price }}"
                   data-ts="{{ $p->created_at ? $p->created_at->timestamp : 0 }}"
                   data-name="{{ strtolower($p->name) }}"
                   data-qv='{{ $qvData }}'>
      ...
      <div x-show="noResults" class="text-center py-20">
        ...
        <button @click="search=''; filterCat=''; priceFilter=''; onSaleFilter=false; stockFilter=false; priceMin=0; priceMax=0"
...
<div x-show="filterOpen" x-cloak class="xl:hidden fixed inset-0 z-50 flex flex-col justify-end">
  ...
  <div class="relative bg-white rounded-t-3xl shadow-2xl max-h-[80vh] flex flex-col"
    ...
    <div class="overflow-y-auto flex-1 px-5 py-4 space-y-6">
      ...
      <div>
        ...
        <div class="space-y-2">
          ...
          <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer transition"
                 :class="stockFilter ? 'border-[var(--c)] bg-[color-mix(in_srgb,var(--c)_8%,white)]' : ''">
            <input type="checkbox" x-model="stockFilter" class="accent-[var(--c)] w-4 h-4 rounded">
            <span class="text-sm font-medium text-gray-700">Solo con stock disponible</span>
          </label>
        </div>
      </div>
    ...
    <div class="flex gap-3 px-5 py-4 border-t border-gray-100 flex-shrink-0">
      <button @click="priceFilter=''; priceMin=0; priceMax=0; onSaleFilter=false; stockFilter=false; sortBy='default'"
  ...
  return {
    ...
    get visibleCount() {
        ...
        return this.searchIndex.filter(p => {
            ...
            const stm = !this.stockFilter || (p.stock === null || p.stock > 0);
            return nm && cm && pm && sm && stm;
        }).length;
    },
    get suggestions() {
    ...
    matchProduct(name, price, comparePrice, stock) {
      ...
      const matchStock = !this.stockFilter  || (stock === null || stock === undefined || stock > 0);
      return matchSearch && matchPrice && matchSale && matchStock;
    },
