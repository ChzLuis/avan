...
@php
    ...
    $categoryIconPaths = $categoryIconPaths ?? \App\Support\IconosCategoria::paths();
    $autoCategoryIcon = $autoCategoryIcon ?? (static fn ($n) => \App\Support\IconosCategoria::auto($n));
    $mmCats = $navCategories->take(max(4, min(14, (int) ($settings['mega_max_cats'] ?? 12))))->values();
    $mmBase = \App\Support\StorefrontNavigation::publicUrl($project);
    $mmShop = \App\Support\StorefrontNavigation::shopUrl($project);
    $mmUrl = static fn ($u) => str_starts_with((string) $u, '/') ? rtrim($mmBase, '/').$u : (string) $u;
    $mmBrands = collect();
    if (($settings['mega_brands'] ?? '1') !== '0') {
        try { $mmBrands = app(\App\Storefront\CatalogQueryService::class)->marcas($project)->take(max(2, min(9, (int) ($settings['mega_brands_max'] ?? 6)))); } catch (\Throwable $e) { $mmBrands = collect(); }
    }
    $mmQuick = [];
...
<div class="mmc" x-show="catsOpen" x-cloak x-transition.opacity.duration.150ms x-data="{ mmCat: {{ (int) $mmCats->first()->id }} }" @mouseleave="catsOpen=false" role="region" aria-label="Categorías del catálogo">
    <div class="mmc-grid{{ $mmSide ? '' : ' no-side' }}">
        <nav class="mmc-cats" aria-label="Rubros">
            ...
            <a class="mmc-cat" :class="mmCat==={{ $cat->id }} && 'is-on'" href="{{ \App\Support\StorefrontNavigation::categoryUrl($project, $cat->id) }}" @mouseenter="mmCat={{ $cat->id }}" @focus="mmCat={{ $cat->id }}">
        ...
        <div class="mmc-main">
            ...
            <div class="mmc-pane" x-show="mmCat==={{ $cat->id }}" @if(!$loop->first) x-cloak @endif>
                ...
                <div class="mmc-subs{{ $mmSubs->count() > 8 ? ' cols-3' : '' }}">
                    ...
                    <a class="mmc-sub" href="{{ \App\Support\StorefrontNavigation::categoryUrl($project, $sub->id) }}">
                ...
                @endif
                <a class="mmc-all" href="{{ \App\Support\StorefrontNavigation::categoryUrl($project, $cat->id) }}">{{ $mmAllText }} <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"/></svg></a>
            </div>
            @endforeach
        </div>
        @if($mmSide)
