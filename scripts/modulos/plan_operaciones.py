# -*- coding: utf-8 -*-
# Plan de Operaciones/ (tras los 8 modulos): agenda y citas, mesas, reservas,
# delivery, mapa operativo y el flujo de lavanderia. `Sede` es Settings (Core).
# Hereda todo el metodo de plan_ventas.py (incluido el paso 3b).
...
T = 'app/Modules/Operaciones'
for c in ['AgendaController', 'DeliveryController', 'MesaController', 'OperationalMapController', 'ReservaController']:
    mv(f'app/Http/Controllers/{c}.php', f'{T}/Controllers/{c}.php')
for m in ['Appointment', 'Availability', 'BlockedDate', 'OperationalEvent', 'OperationalMap', 'OperationalObject', 'OperationalRequest']:
    mv(f'app/Models/{m}.php', f'{T}/Models/{m}.php')
mv('app/Support/LaundryFlow.php', f'{T}/Support/LaundryFlow.php')
mv('app/Console/Commands/CheckLaundryOverdue.php', f'{T}/Commands/CheckLaundryOverdue.php')
V = 'resources/views'
VISTAS = [
    (f'{V}/agenda', f'{T}/Views/agenda', 'agenda'),
    (f'{V}/mapa',   f'{T}/Views/mapa',   'mapa'),
    (f'{V}/comercial/delivery.blade.php', f'{T}/Views/comercial/delivery.blade.php', 'comercial.delivery'),
    (f'{V}/comercial/mesas.blade.php',    f'{T}/Views/comercial/mesas.blade.php',    'comercial.mesas'),
    (f'{V}/comercial/reservas.blade.php', f'{T}/Views/comercial/reservas.blade.php', 'comercial.reservas'),
]
    ...
    for m in DISCO.finditer(s_):
        ...
        for src, dst, _ in VISTAS:
            if rel == src or rel.startswith(src + '/'):
                old = m.group(0)
                new = "app_path('Modules/Operaciones/Views/" + m.group(1)
...
with io.open(os.path.join(L, 'scripts/modulos/plan_operaciones.json'), 'w', encoding='utf-8') as f:
