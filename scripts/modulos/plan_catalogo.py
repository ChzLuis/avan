# -*- coding: utf-8 -*-
# Genera el plan de la mudanza de Catalogo/ (modulo 7/8): productos,
# categorias, servicios, variantes, listas de catalogo, combos, plantillas de
# imagen, integraciones de catalogo (app/Catalog -> Conectores/), comandos y
# jobs de imagenes y sincronizacion. Escribe plan_catalogo.json.
#
# Hereda de plan_tienda.py la clasificacion de literales por contexto y el
# renombrado por nombre corto. Regla mas estricta para los literales sueltos:
# `catalog.*` es sobre todo nombre de PERMISO (catalog.ver, catalog.editar) y
# de checklist; solo se renombra un literal suelto si coincide con una vista
# que exista de verdad en las carpetas movidas.
...
T = 'app/Modules/Catalogo'
for c in ['CatalogIntegrationController', 'CategoryController', 'ProductController', 'ProductVariantController', 'ServiceController']:
    mv(f'app/Http/Controllers/Catalog/{c}.php', f'{T}/Controllers/{c}.php')
for c in ['CatalogListController', 'ComboController', 'ProductImageTemplateController']:
    mv(f'app/Http/Controllers/{c}.php', f'{T}/Controllers/{c}.php')
for m in ['Product', 'ProductAttribute', 'ProductAttributeValue', 'ProductImage', 'ProductImageTemplate', 'ProductVariant',
          'Category', 'Service', 'CatalogList', 'CatalogValue', 'CatalogIntegration', 'CatalogIntegrationItem',
          'CatalogSyncRun', 'ImportLog', 'Combo', 'ComboItem']:
    mv(f'app/Models/{m}.php', f'{T}/Models/{m}.php')
for s_ in ['EtiquetasProducto', 'UnidadesMedida', 'SquareImage']:
    mv(f'app/Support/{s_}.php', f'{T}/Support/{s_}.php')
for dp, dn, fn in os.walk(os.path.join(L, 'app/Catalog')):
    for f in sorted(fn):
        if f.endswith('.php'):
            rel = os.path.relpath(os.path.join(dp, f), os.path.join(L, 'app/Catalog')).replace(B, '/')
            mv(f'app/Catalog/{rel}', f'{T}/Conectores/{rel}')
mv('app/Providers/CatalogServiceProvider.php', f'{T}/CatalogServiceProvider.php')
for c in ['SyncCatalogCommand', 'GenerarSkuProductos', 'ImportarImagenesExternas', 'RegenerarImagenes']:
    mv(f'app/Console/Commands/{c}.php', f'{T}/Commands/{c}.php')
for j in ['GenerarImagenesProducto', 'RunCatalogSync']:
    mv(f'app/Jobs/{j}.php', f'{T}/Jobs/{j}.php')

V = 'resources/views'
VISTAS = [
    (f'{V}/catalog',  f'{T}/Views/catalog',  'catalog'),
    (f'{V}/catalogs', f'{T}/Views/catalogs', 'catalogs'),
    (f'{V}/combos',   f'{T}/Views/combos',   'combos'),
    (f'{V}/components/etiquetas-producto.blade.php', f'{T}/Views/components/etiquetas-producto.blade.php', 'components.etiquetas-producto'),
]
for src, dst, _ in VISTAS:
    if not os.path.exists(os.path.join(L, src)): raise SystemExit('no existe ' + src)

# Vistas que existen de verdad (para los literales sueltos).
VISTAS_REALES = set()
...
OTROS = {}
for mod in ('Personas', 'Control', 'Inventario', 'Finanzas', 'Crm', 'Bots', 'Tienda'):
...
# 5) EXTRA: rutas de disco.
def extra(p, old, new, n=1):
    add_edit(destino(p), old, new, n)
extra('tests/Feature/EtiquetasMasivasTest.php', "resource_path('views/catalog/products/index.blade.php')", "app_path('Modules/Catalogo/Views/catalog/products/index.blade.php')")

# 6) Avisos: includes de componentes por nombre de vista y namespace viejo suelto.
avisos = []
for p, s in contenido.items():
    if 'App' + B + 'Catalog' + B + '{' in s: avisos.append(f'{p}: group use de App{B}Catalog')
    if re.search(r'views/(catalog|catalogs|combos)', s) and 'app/Modules' not in p: avisos.append(f'{p}: ruta de disco a vistas de catalogo')

plan = {'moves': moves + [(s, d) for s, d, _ in VISTAS], 'edits': edits, 'inserts': inserts, 'decisiones': decisiones, 'avisos': avisos}
with io.open(os.path.join(L, 'scripts/modulos/plan_catalogo.json'), 'w', encoding='utf-8') as f:
