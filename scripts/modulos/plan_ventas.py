# -*- coding: utf-8 -*-
# Genera el plan de la mudanza de Ventas/ (modulo 8/8, el mas entrelazado):
# pedidos, cotizaciones, propuestas, POS, portal del cliente, revendedores,
# reportes comerciales, WooSync y tickets. Ademas repara un resto de Finanzas:
# `Cobranza` (CxC) pasa a Finanzas/Support. Escribe plan_ventas.json.
#
# Se quedan en Core la CARA del portal comercial (Comercial\Auth/Dashboard,
# comercial/layouts, login, dashboard, panel/), DashboardController,
# AvisosPortal (lo consume el layout), BusinessTerms (QR), LaundryFlow y
# CheckLaundryOverdue (Operaciones, pendiente), Client (CRM, paso aparte).
#
# Novedad: las lecturas de vistas por RUTA DE DISCO en tests/app se reescriben
# automaticamente (resource_path('views/<movida>') -> app_path('Modules/...')).
...
T = 'app/Modules/Ventas'
for c in ['OrderController', 'QuoteController', 'PosController', 'ProposalController', 'PortalController',
          'PortalClienteController', 'ResellerController', 'WooSyncController', 'ReporteController',
          'DashboardComercialController', 'TicketsWpController']:
    mv(f'app/Http/Controllers/{c}.php', f'{T}/Controllers/{c}.php')
for m in ['Order', 'OrderItem', 'OrderEvent', 'Quote', 'QuoteItem', 'Proposal', 'ResellerPrice', 'SalesInteraction', 'AbandonedCart']:
    mv(f'app/Models/{m}.php', f'{T}/Models/{m}.php')
for s_ in ['OrderFlow', 'OrderStatus', 'OrderAbilities', 'QuoteAbilities', 'QuoteStatus']:
    mv(f'app/Support/{s_}.php', f'{T}/Support/{s_}.php')
mv('app/Support/Cobranza.php', 'app/Modules/Finanzas/Support/Cobranza.php')

V = 'resources/views'
VISTAS = [
    (f'{V}/orders',         f'{T}/Views/orders',         'orders'),
    (f'{V}/quotes',         f'{T}/Views/quotes',         'quotes'),
    (f'{V}/pos',            f'{T}/Views/pos',            'pos'),
    (f'{V}/proposals',      f'{T}/Views/proposals',      'proposals'),
    (f'{V}/reseller',       f'{T}/Views/reseller',       'reseller'),
    (f'{V}/portal-cliente', f'{T}/Views/portal-cliente', 'portal-cliente'),
    (f'{V}/ventas',         f'{T}/Views/ventas',         'ventas'),
    (f'{V}/dashboard/comercial.blade.php', f'{T}/Views/dashboard/comercial.blade.php', 'dashboard.comercial'),
    (f'{V}/comercial/reportes',            f'{T}/Views/comercial/reportes',            'comercial.reportes'),
    (f'{V}/comercial/woo-orders.blade.php', f'{T}/Views/comercial/woo-orders.blade.php', 'comercial.woo-orders'),
    (f'{V}/comercial/tickets-wp.blade.php', f'{T}/Views/comercial/tickets-wp.blade.php', 'comercial.tickets-wp'),
    (f'{V}/facturacion/pedidos',      f'{T}/Views/facturacion/pedidos',      'facturacion.pedidos'),
    (f'{V}/facturacion/cotizaciones', f'{T}/Views/facturacion/cotizaciones', 'facturacion.cotizaciones'),
]
...
OTROS = {}
for mod in ('Personas', 'Control', 'Inventario', 'Finanzas', 'Crm', 'Bots', 'Tienda', 'Catalogo'):
    ...
    for m in DISCO.finditer(s_):
        ...
        for src, dst, _ in VISTAS:
            if rel == src or rel.startswith(src + '/'):
                old = m.group(0)
                new = "app_path('Modules/Ventas/Views/" + m.group(1)
...
avisos = []
for p, s in contenido.items():
    if re.search(r'views/(orders|quotes|pos|proposals|reseller|portal-cliente|ventas)', s) and 'app/Modules' not in p and 'resource_path' not in s: avisos.append(f'{p}: menciona vistas de ventas por ruta sin resource_path')
    

plan = {'moves': moves + [(s, d) for s, d, _ in VISTAS], 'edits': edits, 'inserts': inserts, 'decisiones': decisiones, 'avisos': avisos}
with io.open(os.path.join(L, 'scripts/modulos/plan_ventas.json'), 'w', encoding='utf-8') as f:
