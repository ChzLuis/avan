# -*- coding: utf-8 -*-
# Aplica un plan de mudanza (plan_*.json generado por plan_*.py).
#   python scripts/modulos/mover_modulo.py scripts/modulos/plan_X.json [--dry]
#
...
PLAN = os.path.join(L, [a for a in sys.argv[1:] if not a.startswith('--')][0]) if [a for a in sys.argv[1:] if not a.startswith('--')] else os.path.join(L, 'scripts/modulos/plan_finanzas.json')
DRY = '--dry' in sys.argv
B = chr(92)
