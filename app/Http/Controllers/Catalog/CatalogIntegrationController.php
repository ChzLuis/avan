...
use App\Catalog\Sync\CatalogSyncManager;
use App\Http\Controllers\Controller;
use App\Jobs\RunCatalogSync;
...
class CatalogIntegrationController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('catalog.integrations.index', [
            'project' => $project,
    ...
    public function preview(int $integration)
    {
        ...
        if (!($provider instanceof \App\Catalog\Contracts\ProvidesProducts)) {
            return response()->json(['ok' => false, 'message' => 'Este proveedor no expone productos.'], 422);
        }

        $page = $provider->fetchProducts($integration, \App\Catalog\DTOs\SyncCursor::start());
