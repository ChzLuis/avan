...
class ConversacionesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $cnts = [
            'todos'      => DB::table('bot_sessions')->count(),
            'completado' => DB::table('bot_sessions')->where('current_state', 'confirmacion_final')->count(),
            'en_proceso' => DB::table('bot_sessions')->whereNotIn('current_state', ['confirmacion_final', 'menu_principal', 'inicio'])->count(),
            'inactivo'   => DB::table('bot_sessions')->whereIn('current_state', ['menu_principal', 'inicio'])->count(),
        ];
    ...
    public function mensajes($id)
    {
        ...
        $sesion = DB::table('bot_sessions')->where('id', $id)->first();
