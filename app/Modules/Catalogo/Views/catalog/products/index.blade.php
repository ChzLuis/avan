 ...
 * `App\Modules\Catalogo\Support\EtiquetasProducto`, que vuelve a sanearlo todo al guardar.
 */
function etiquetasProducto() {
    return {
        catalogo: {!! Illuminate\Support\Js::from(collect(\App\Modules\Catalogo\Support\EtiquetasProducto::catalogo())->map(fn ($e) => [
            'etiqueta' => $e['texto'], 'fondo' => $e['fondo'], 'texto' => $e['texto_color'], 'prioridad' => $e['prioridad'],
        ])) !!},
        maxCard: {{ \App\Modules\Catalogo\Support\EtiquetasProducto::MAX_EN_CARD }},
...
window.__productPageData = {
    products:   {!! Illuminate\Support\Js::from($products->map(fn($p) => [
        ...
        'etiquetas'        => json_encode(\App\Modules\Catalogo\Support\EtiquetasProducto::config($p), JSON_UNESCAPED_UNICODE),
                ...
                <div x-show="openBulk && view==='etiquetas'" x-cloak
                    ...
                    <div class="max-h-56 overflow-y-auto rounded-lg border border-gray-200 p-2">
                        <div class="grid grid-cols-2 gap-x-2 gap-y-1">
                            @foreach(\App\Modules\Catalogo\Support\EtiquetasProducto::catalogo() as $bk => $be)
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                <div x-show="tab==='info'" x-cloak class="p-6 max-w-3xl space-y-4">
                    ...
                    <div class="pe-card p-5" x-data="etiquetasProducto()">
                        ...
                        <p class="pe-hint mb-3">Distintivos comerciales que aparecen sobre la foto. En el catálogo se muestran <strong>máximo {{ \App\Modules\Catalogo\Support\EtiquetasProducto::MAX_EN_CARD }}</strong> (las de mayor prioridad); en la ficha se ven todas.</p>
                        ...
                        <div class="max-h-56 overflow-y-auto rounded-lg border border-gray-200 p-3">
                            <div class="grid gap-x-4 gap-y-1.5 sm:grid-cols-2">
                                @foreach(\App\Modules\Catalogo\Support\EtiquetasProducto::catalogo() as $clave => $e)
                        ...
                        <details class="mt-3 rounded-lg border border-gray-200 p-3">
                            ...
                            <div class="mt-3 grid gap-3 sm:grid-cols-2">
                                ...
                                <label class="text-xs font-semibold text-gray-600">Posición
                                    <select x-model="etq.personalizada.posicion" @change="sync()" class="mt-1 w-full rounded border-gray-300 text-sm">
                                        @foreach(\App\Modules\Catalogo\Support\EtiquetasProducto::POSICIONES as $pk => $pl)
                            ...
                            <p class="pe-hint mt-2">Sin texto no se crea ninguna etiqueta. La prioridad decide cuál gana cuando hay más de {{ \App\Modules\Catalogo\Support\EtiquetasProducto::MAX_EN_CARD }}.</p>
                        </details>
                    </div>
                ...
                <div x-show="tab==='precios'" x-cloak class="p-6 max-w-3xl space-y-4">
                    ...
                    <div class="rounded-xl overflow-hidden border {{ $priceConfig['header_border'] }} shadow-sm">
                        ...
                        @php
                            ...
                            $gruposUnidad = \App\Modules\Catalogo\Support\UnidadesMedida::paraNegocio($pCat, $units->pluck('label')->all());
                        @endphp
                            ...
                            <div>
                                ...
                                <select x-model="form.wholesale_unit" class="pe-input"
                                    ...
                                    <optgroup label="Unidades de medida">
                                        @foreach(\App\Modules\Catalogo\Support\UnidadesMedida::todas() as $u)
                ...
                @include('catalogo::catalog.products.partials.variants')
