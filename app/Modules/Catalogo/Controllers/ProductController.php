...
class ProductController extends Controller
{
    public function index()
    {
        ...
        return view('catalogo::catalog.products.index', compact('project', 'categories', 'products', 'brands', 'units', 'suppliers', 'locations', 'taxes', 'allCatalogs', 'pdfProfiles', 'usaVariantes'));
    }
    ...
    public function catalogPdf(Request $request)
    {
        ...
        return view('catalogo::catalog.products.catalog-pdf', compact(
    ...
    private function applyEtiquetas(array $data, ?Product $product = null): array
    {
        ...
        $limpias = is_array($entrada) ? \App\Modules\Catalogo\Support\EtiquetasProducto::normaliza($entrada) : null;
    ...
    public function purgeAll(Request $request)
    {
        ...
        if ($count > 0) {
            \App\Modules\Catalogo\Models\ProductImage::whereIn('product_id', $productIds)->get()->each(function (ProductImage $image) {
                $relativePath = ltrim(str_replace('/avan/public/', '', parse_url($image->url, PHP_URL_PATH)), '/');
                $fullPath = public_path($relativePath);
                if (file_exists($fullPath)) @unlink($fullPath);
            });

            \App\Modules\Catalogo\Models\CatalogIntegrationItem::where('local_type', Product::class)
    ...
    private function bulkEtiquetas($query, array $data): int
    {
        ...
        foreach ($query->get() as $producto) {
            $actual = \App\Modules\Catalogo\Support\EtiquetasProducto::config($producto);
            ...
            $limpias = \App\Modules\Catalogo\Support\EtiquetasProducto::normaliza($entrada);
            if ($limpias) $options['etiquetas'] = $limpias;
            else unset($options['etiquetas']);
    ...
    private function bulkSetCategory($query, $project, ?int $categoryId): int
    {
        if ($categoryId !== null) {
            ...
            abort_unless(
                \App\Modules\Catalogo\Models\Category::where('id', $categoryId)->where('project_id', $project->id)->exists(),
    ...
    public function exportStatic()
    {
        ...
        $html = view('catalogo::catalog.products.export-static', compact('project', 'categories', 'settings', 'waRaw'))->render();
    ...
    public function reviews()
    {
        ...
        return view('catalogo::catalog.reviews.index', compact('reviews'));
    }
    ...
    {
        try {
            $plantilla = \App\Modules\Catalogo\Models\ProductImageTemplate::allProjects()
            ...
            \App\Modules\Catalogo\Jobs\GenerarImagenesProducto::dispatch($plantilla->id, [$image->id], false);
        } catch (\Throwable $e) {
