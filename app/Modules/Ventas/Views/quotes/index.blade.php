...
<div class="q-wrap" x-init="montarHistorial(); setInterval(() => ahora = Date.now(), 30000)" x-data="{
    quotes: {{ Js::from($quotes->map(fn($q) => [
        ...
        'status'           => \App\Modules\Ventas\Support\QuoteStatus::comercial($q->status),
        'payment_status'   => \App\Modules\Ventas\Support\QuoteStatus::pago($q->payment_status),
        'pill_comercial'   => \App\Modules\Ventas\Support\QuoteStatus::comercialPresentacion($q->status),
        'pill_pago'        => \App\Modules\Ventas\Support\QuoteStatus::pagoPresentacion($q->payment_status),
        'vencida'          => \App\Modules\Ventas\Support\QuoteStatus::vencida($q->status, $q->valid_until),
...
<div class="q-main" :class="panel==='list' ? 'hidden md:flex' : 'flex'">
    ...
    <div class="q-doc" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
        ...
        @include('ventas::quotes._acciones')
