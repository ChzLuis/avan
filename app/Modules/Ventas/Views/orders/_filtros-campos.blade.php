...
<select x-model="f.comercial" aria-label="Estado comercial" class="pf-select">
    ...
    @foreach (\App\Modules\Ventas\Support\OrderStatus::opcionesComercial() as $valor => $etiqueta)
...
<select x-model="f.pago" aria-label="Estado de pago" class="pf-select">
    ...
    @foreach (\App\Modules\Ventas\Support\OrderStatus::opcionesPago() as $valor => $etiqueta)
        <option value="{{ $valor }}">{{ $etiqueta }}</option>
    @endforeach
</select>
