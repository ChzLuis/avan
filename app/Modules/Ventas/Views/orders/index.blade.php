...
<div class="flex flex-col flex-1 overflow-hidden bg-gray-50" x-init="initPedidos()" x-data="{
    ...
    orders: {{ Js::from($orders->map(function($o) use ($project, $conFlujo) {
        $sla = $conFlujo && $o->laundry_status ? \App\Modules\Ventas\Support\OrderFlow::slaStatus($project, $o) : null;
        return [
        'id'             => $o->id,
        ...
        'pill_comercial' => \App\Modules\Ventas\Support\OrderStatus::comercialPresentacion($o->status),
        'pill_pago'      => \App\Modules\Ventas\Support\OrderStatus::pagoPresentacion($o->payment_status),
        'debe'           => \App\Modules\Ventas\Support\OrderStatus::debe($o->status, $o->payment_status),
        'comercial_key'  => \App\Modules\Ventas\Support\OrderStatus::comercialCanonico($o->status),
        'pago_key'       => \App\Modules\Ventas\Support\OrderStatus::pago($o->payment_status),
...
@include('ventas::orders._filtros')

{{-- ══ Listado a ancho completo ═══════════════════════════════════════════ --}}
<div class="flex flex-col flex-1 overflow-hidden bg-white">
    @include('ventas::orders._fila')
</div>

@include('ventas::orders._drawer')
</div>
