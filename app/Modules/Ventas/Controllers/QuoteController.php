...
class QuoteController extends Controller
{
    public function index(?Quote $cotizacionInicial = null)
    {
        ...
        $puede             = \App\Modules\Ventas\Support\QuoteAbilities::para(auth()->user(), $project);
        ...
        return view('ventas::quotes.index', compact(
    ...
    public function store(Request $request)
    {
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'created', [], null, $quote->id);
    ...
    {
        return \App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status) === 'converted';
    }
    ...
    private function estadoDestinoValidado(?string $destino, Quote $quote): string
    {
        ...
        abort_if(\App\Modules\Ventas\Support\QuoteStatus::comercial($destino) === 'converted', 422,
    ...
    public function update(Request $request, Quote $quote)
    {
        ...
        // --- Ciclo de vida F1c -------------------------------------------
        $destino = \App\Modules\Ventas\Support\QuoteStatus::comercial($data['status']);
        ...
        if (array_key_exists('payment_status', $data)) {
            ...
            $data['payment_status'] = \App\Modules\Ventas\Support\QuoteStatus::pago($data['payment_status']);
        }
        ...
        if (array_key_exists('payment_status', $data) && $data['payment_status'] !== $quote->payment_status) {
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'payment_status', ['from' => $quote->payment_status, 'to' => $data['payment_status']], null, $quote->id);
        }
        ...
        // cual escribio quien.
        $estadoPrevio = \App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status);
        $quote->update($data);
        if ($destino !== $estadoPrevio) {
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'status_changed',
    ...
    private function registrarEdicion(Project $project, Quote $quote, array $antes, array $despues): void
    {
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'edited', [
    ...
    public function send(Quote $quote)
    {
        ...
        abort_unless(in_array(\App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status), ['draft', 'sent'], true), 422,
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'quote_sent', [], null, $quote->id);
    ...
    public function pdf(Quote $quote)
    {
        ...
        return view('ventas::quotes.pdf', compact('project', 'quote'));
    }
    ...
    public function events(Quote $quote)
    {
        ...
        $eventos = \App\Modules\Ventas\Models\OrderEvent::with('user:id,name')
    ...
    public function destroy(Quote $quote)
    {
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'deleted', [
            'numero'   => $quote->etiqueta,
            'quote_id' => $quote->id,
            'cliente'  => $quote->client_name,
            'total'    => \App\Support\LineMath::canon((string) $quote->total),
            'estado'   => \App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status),
        ]);
    ...
    public function convert(Quote $quote)
    {
        ...
        try {
            ...
            if ($esUnique) {
                $ganador = \App\Modules\Ventas\Models\Order::where('quote_id', $quote->id)->first();
    ...
    private function convertTx(Quote $quote, $project)
    {
        return \Illuminate\Support\Facades\DB::transaction(function () use ($quote, $project) {
            ...
            if (\App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status) === 'converted') {
                $existente = \App\Modules\Ventas\Models\Order::where('quote_id', $quote->id)->first();
                ...
                if (! $existente) {
                    ...
                    $candidatos = \App\Modules\Ventas\Models\OrderEvent::where('quote_id', $quote->id)
                    ...
                    if ($candidatos->count() === 1) {
                        ...
                        $ambiguoA = \App\Modules\Ventas\Models\OrderEvent::where('order_id', $orderId)
                        ...
                        $candidato = $ambiguoA ? null
                            : \App\Modules\Ventas\Models\Order::where('project_id', $quote->project_id)
                                ->find($orderId);

                        if ($candidato) {
                            // whereNull: si otra peticion fijo la FK entre
                            // medias, no se pisa; se relee y usa lo fijado.
                            \App\Modules\Ventas\Models\Order::whereKey($candidato->id)
                                ->whereNull('quote_id')
            ...
            $desalineado = \App\Modules\Ventas\Models\Order::where('quote_id', $quote->id)->first();
            if ($desalineado) {
            ...
            abort_unless(\App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status) === 'accepted', 422,
                'Solo se convierte en pedido una cotización aceptada por el cliente. '
                . 'Esta está en estado "' . \App\Modules\Ventas\Support\QuoteStatus::comercialPresentacion($quote->status)['label'] . '".');
            ...
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'converted', ['order_id' => $order->id], $order->id, $quote->id);
    ...
    public function indexPortal(string $slug)
    {
        ...
        return view('ventas::facturacion.cotizaciones.index', compact('project', 'quotes', 'showBase', 'updateBase', 'boletaCreateUrl', 'facturaCreateUrl'));
    }
    ...
    public function createPortal(string $slug)
    {
        ...
        return view('ventas::facturacion.cotizaciones.create', compact('project', 'clients', 'catalogo', 'rucUrl'));
    }
    ...
    public function editPortal(string $slug, $id)
    {
        ...
        $rucUrl           = route('facturacion.ruc.lookup', $slug);
        return view('ventas::facturacion.cotizaciones.edit', compact(
