...
class PortalController extends Controller
{
    ...
    public function accept(Request $request, string $slug, string $token)
    {
        ...
        if (!in_array(\App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status), ['sent', 'draft'])) { // normaliza 'borrador' legacy
        ...
        if (\App\Modules\Ventas\Support\QuoteStatus::vencida($quote->status, $quote->valid_until)) {
        ...
        $resultado = \Illuminate\Support\Facades\DB::transaction(function () use ($quote, $project, $request) {
            ...
            if (! $fresca || ! in_array(\App\Modules\Ventas\Support\QuoteStatus::comercial($fresca->status), ['sent', 'draft'], true)) {
                return false;   // otra peticion llego primero
            }
            if (\App\Modules\Ventas\Support\QuoteStatus::vencida($fresca->status, $fresca->valid_until)) {
                return false;
            }
            ...
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'accepted_by_client', [], null, $fresca->id);
    ...
    public function reject(Request $request, string $slug, string $token)
    {
        ...
        $resultado = \Illuminate\Support\Facades\DB::transaction(function () use ($quote, $project, $reason) {
            ...
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'rejected_by_client', ['reason' => $reason ?: null], null, $fresca->id);
    ...
    public function proof(Request $request, string $slug, string $token)
    {
        ...
        if (! in_array(\App\Modules\Ventas\Support\QuoteStatus::comercial($quote->status), ['accepted', 'converted'], true)) {
        ...
        $quote->update([
            ...
            'payment_status'    => \App\Modules\Ventas\Support\QuoteStatus::pago($quote->payment_status),
        ]);
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'proof_uploaded', [], null, $quote->id);
