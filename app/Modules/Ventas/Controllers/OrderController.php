...
use App\Modules\Ventas\Support\OrderStatus;
...
class OrderController extends Controller
{
    public function index()
    {
        ...
        $cerrados = \App\Modules\Ventas\Support\OrderStatus::rawsCerrados();
        $pendientes = \App\Modules\Ventas\Support\OrderStatus::rawsDe('pending');
        ...
        $kpis = [
            ...
            'completados'=> $project->orders()->whereIn('status', \App\Modules\Ventas\Support\OrderStatus::rawsDe('done'))->count(),
        ];
        ...
        $puede = \App\Modules\Ventas\Support\OrderAbilities::para(auth()->user());
        ...
        $flujoOperativo = $capacidadOperativa && \App\Modules\Ventas\Support\OrderFlow::supportsFlow($project->category ?? '')
            ? \App\Modules\Ventas\Support\OrderFlow::activeStates($project)
            : [];

        // Pedido preseleccionado al entrar por /bixosales/pedidos/{order}.
        $pedidoInicial = request()->route('order')?->id;

        return view('ventas::orders.index', compact(
    ...
    public function pdf(Order $order)
    {
        ...
        return view('ventas::orders.pdf', compact('order', 'project'));
    }
    ...
    public function tag(Order $order)
    {
        ...
        return view('ventas::orders.tag', compact('order', 'project', 'qrUrl'));
    }
    ...
    public function update(Request $request, Order $order)
    {
        ...
        if (($data['status'] ?? $order->status) !== $order->status) {
            \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'status_changed', ['from' => $order->status, 'to' => $data['status']], $order->id);
        }
        $order->update($data);
    ...
    public function issueDocument(Request $request, Order $order)
    {
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'document_issued', ['type' => $data['type'], 'number' => $data['number'] ?? null], $order->id);
    ...
    public function events(Order $order)
    {
        $project = $this->proyectoActivo();
        abort_unless($order->project_id === $project->id, 403);
        $events = \App\Modules\Ventas\Models\OrderEvent::where('order_id', $order->id)->with('user:id,name')
    ...
    public function waSent(Request $request, Order $order)
    {
        ...
        \App\Modules\Ventas\Models\OrderEvent::log($project->id, 'whatsapp_sent', $data, $order->id);
    ...
    public function indexPortal(string $slug)
    {
        ...
        $paymentConditions = $this->catValues($project, 'payment_condition');
        $salesChannels     = $this->catValues($project, 'sales_channel');
        return view('ventas::facturacion.pedidos.index', compact('project', 'orders', 'paymentMethods', 'paymentConditions', 'salesChannels'));
    }
    ...
    public function kitchen()
    {
        ...
        return view('ventas::orders.kitchen', compact('orders', 'ordersJson', 'project'));
    }
