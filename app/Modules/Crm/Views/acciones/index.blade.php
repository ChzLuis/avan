        ...
        <template x-for="a in grupo(pestana)" :key="a.id">
            <div class="bg-white rounded-2xl border px-3 py-2.5 flex items-start gap-3" :class="a.vencida && !a.hecho_at ? 'border-red-200' : 'border-gray-200'">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-gray-900" :class="a.hecho_at ? 'line-through text-gray-400' : ''" x-text="a.titulo"></p>
...
function acciones() {
    ...
    return {
        ...
        async guardar() {
            ...
            this.guardando = true;
            const r = await fetch('/bixocrm/acciones', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf(), Accept: 'application/json' }, body: JSON.stringify({ titulo: this.form.titulo, vence_at: this.form.vence_at || null }) });
            const d = await r.json().catch(() => ({}));
            if (d.ok) { this.acciones.unshift(d.accion); this.form = { titulo: '', vence_at: '' }; this.nueva = false; }
