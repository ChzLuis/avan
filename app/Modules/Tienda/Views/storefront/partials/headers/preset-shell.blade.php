...
@php
    ...
    $currentCategory = \App\Modules\Tienda\Support\StorefrontNavigation::currentCategoryId();
...
@php
    ...
    $shTopHelpUrl = trim((string) $hp('topbar_help_url', ''))
        ?: \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'contact']));
@endphp
@if($shTopRich)
...
<div class="hpx-uni-top">
    <div class="container hpx-uni-top-inner">
        ...
        <a class="hpx-uni-chip {{ empty($activeProfile) && !$currentCategory ? 'is-active' : '' }}" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}">General</a>
        @if($shUniProfiles)
            ...
            <a class="hpx-uni-chip {{ (!empty($activeProfile) && $activeProfile->id === $cp->id) ? 'is-active' : '' }}"
               ...
               href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, $cp->slug) }}">{{ $cp->menu_label ?: $cp->name }}</a>
            @endforeach
        @else
...
<header class="store-header">
    <div class="container hpx-main hpx-logo-{{ $shLogo }} hpx-search-{{ $shSearch }}">
        ...
        @if($shLogo === 'center')
        <div class="hpx-aux">
            @if($menuRoots->count())@foreach($menuRoots->take(2) as $auxItem)<a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, $auxItem) }}">{{ $auxItem->label }}</a>@endforeach @endif
        </div>
        @endif

        <a class="brand {{ $logoUrl ? 'brand--logo-only' : 'brand--text' }}" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}" aria-label="Inicio de {{ $storeName }}">
        ...
        <nav class="hpx-inline-nav" aria-label="Navegación">
            ...
            <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, $mItem) }}">{{ $mItem->label }}</a>
            @endforeach
        </nav>
        @endif
        ...
        @if($shLogo !== 'center')
            ...
            <div class="hpx-searchbox {{ $shSearchCats ? 'has-cats' : '' }}">
                @include('tienda::storefront.partials.nav.search-box')
                @if($shSearchCats)
...
<nav class="category-nav hpx-nav hpx-nav-{{ $shNavAlign }}" aria-label="Navegación" x-data="{ mega:false, megaCat: {{ $navCategories->first()->id ?? 'null' }}, vsp:false }" @mouseleave="mega=false">
    <div class="container category-bar">
        ...
        @if(in_array($shCatTrigger, ['mega', 'lista'], true) && $navCategories->count())
            @include('tienda::storefront.partials.nav.cats-button', ['modo' => $shCatTrigger, 'pos' => $shCatsPos, 'var' => 'mega'])
        @endif
        ...
        <div class="category-list">
            @if($menuRoots->count())
                @foreach($menuRoots as $mItem)
                    ...
                    @else
                        <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, $mItem) }}" class="{{ $isActive ? 'is-active' : '' }}" @if($mItem->target === '_blank') target="_blank" rel="noopener" @endif>{{ $mItem->label }}</a>
                    @endif
                @endforeach
            @else
        ...
        @php
            ...
            $shChipUrl = function (string $target) use ($project, $shopBase, $shWa, $settings) {
                ...
                if (str_starts_with($target, 'perfil:')) {
                    return \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, substr($target, 7));
                }
    ...
    <div class="mega-panel hpx-mega" data-cols="{{ $hpMegaColsShown }}" data-layout="{{ $shMegaLayout }}" x-show="mega" x-cloak x-transition.opacity @mouseenter="mega=true">
        ...
        <div class="container hpx-mega-lat">
            <div class="hpx-mega-aside" role="list">
                ...
                <a class="hpx-mega-cat" role="listitem"
                   ...
                   :class="megaCat==={{ $cat->id }} && 'is-on'">@if($shMenuIcons !== 'no')<span class="hpx-cat-ico" aria-hidden="true">{!! \App\Modules\Tienda\Support\CategoryIcons::svg($cat->name, $settings['caticon_'.$cat->id] ?? null) !!}</span>@endif<span class="hpx-mega-cat-txt">{{ $cat->name }}</span>@if($cat->children->count())<span class="hpx-mega-cat-arr" aria-hidden="true">›</span>@endif</a>
                @endforeach
            </div>
        ...
        <div class="container hpx-mega-grid" style="--hpx-mc:{{ $hpMegaColsShown }}">
            ...
            <div class="hpx-mega-col">
                ...
                <a class="hpx-mega-title" href="{{ $shopBase }}?category={{ $cat->id }}">@if($shMenuIcons !== 'no')<span class="hpx-cat-ico" aria-hidden="true">{!! \App\Modules\Tienda\Support\CategoryIcons::svg($cat->name, $settings['caticon_'.$cat->id] ?? null) !!}</span>@endif{{ $cat->name }}</a>
                @if($cat->children->count())
            ...
            @endforeach
            @includeIf('tienda::storefront.partials.nav.mega-extras')
        </div>
        @endif
        ...
        <div class="container hpx-ed-grid">
            ...
            <div>
                ...
                @if($edUniverses)
                    ...
                    <a class="hpx-ed-link" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, $cp->slug) }}">{{ $cp->menu_label ?: $cp->name }}</a>
                    @endforeach
                @else
...
<div class="hpx-uni-below">
    <div class="container hpx-uni-below-inner">
        <a class="hpx-uni-pill {{ empty($activeProfile) && !$currentCategory ? 'is-active' : '' }}" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}">Todo</a>
        @if($shUniProfiles)
            @foreach($catalogProfiles as $cp)
            <a class="hpx-uni-pill {{ (!empty($activeProfile) && $activeProfile->id === $cp->id) ? 'is-active' : '' }}" @if($cp->primary_color) style="--chip-color:{{ $cp->primary_color }}" @endif href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, $cp->slug) }}">{{ $cp->menu_label ?: $cp->name }}</a>
            @endforeach
        @else
            @foreach($shUniItems as $uc)
