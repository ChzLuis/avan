...
<div class="header-zone hdr-compact {{ $headerStickyMode !== 'none' ? 'is-sticky' : '' }}">
    <header class="hcp-row" x-data="{ catsOpen: false }" @click.outside="catsOpen=false">
        ...
        <a class="hcp-brand" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'home'])) }}">
        ...
        <nav class="hcp-nav" aria-label="Navegación principal">
            ...
            @empty
            <a class="hcp-link {{ $activeDest === 'home' ? 'is-active' : '' }}" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'home'])) }}">Inicio</a>
