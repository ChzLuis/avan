    ...
    <header class="store-header">
        <div class="container header-main">
            ...
            <a class="brand {{ $logoUrl ? 'brand--logo-only' : 'brand--text' }}"
               href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}"
    ...
    <nav class="category-nav" aria-label="Navegación" x-data="{ mega: false, megaCat: {{ $navCategories->first()->id ?? 'null' }} }" @mouseleave="mega=false">
        <div class="container category-bar menu-{{ $menuAlign }}">
            ...
            @endphp
            @if($clModo !== 'none')
                @include('tienda::storefront.partials.nav.cats-button', ['modo' => $clModo, 'pos' => $clCatsPos, 'var' => 'mega'])
            @endif

            {{-- Menú del Constructor (Inicio, Productos, Nosotros, Contacto) --}}
            <div class="category-list">
            @php $currentCategory = \App\Modules\Tienda\Support\StorefrontNavigation::currentCategoryId(); @endphp
            @if($menuRoots->count())
                @foreach($menuRoots as $mItem)
                    ...
                    @else
                        <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, $mItem) }}"
