...
@php
    ...
    $themePresetDef = \App\Modules\Tienda\Support\StorefrontThemePresets::get($settings['theme_preset'] ?? null);
...
<head>
    ...
    @php
        $bizLegal = \App\Modules\Tienda\Storefront\DatosPie::legal($settings ?? [], $project);
        $bizTel = \App\Modules\Tienda\Storefront\ContactosTienda::todos($settings ?? [], $project);
        ...
        $bizHorario = \App\Modules\Tienda\Storefront\DatosPie::horario($settings['business_hours'] ?? null);
    ...
    <style>
        :root{
          ...
          {!! \App\Modules\Tienda\Storefront\DatosPie::variables(\App\Modules\Tienda\Storefront\DatosPie::paleta(
...
@endif
    ...
    @include('tienda::public.templates.partials.computienda-card-styles')
    @include('tienda::public.templates.partials.computienda-hero-veil')
    @include('tienda::public.templates.partials.computienda-nosotros')
</head>
@php
    // Clases del preset de encabezado (tamaño del buscador y ajustes por modelo)
    $hpBodyKey = \App\Modules\Tienda\Support\HeaderPresets::active($settings);
    $hpBodyClass = $hpBodyKey
        ? 'hp-preset hp-' . str_replace('_', '-', $hpBodyKey) . ' hp-search-' . (\App\Modules\Tienda\Support\HeaderPresets::get($hpBodyKey)['search'] ?? 'normal')
        : '';
@endphp
<body x-data="professionalStore()" x-init="init()" :class="{'ck-focus': checkoutOpen}"
    ...
    @php
        ...
        $shopBase = \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'shop']));
    ...
    @php
        ...
        $hpKey = \App\Modules\Tienda\Support\HeaderPresets::active($settings);
        $hpPreset = \App\Modules\Tienda\Support\HeaderPresets::get($hpKey);
        ...
        if ($hpKey) {
            ...
            $hpCandidate = !empty($hpPreset['shell'])
                ? 'tienda::storefront.partials.headers.preset-shell'
                : 'tienda::storefront.partials.headers.' . (\App\Modules\Tienda\Support\HeaderPresets::baseLayout($hpKey) ?? 'classic');
            $hpHeaderView = view()->exists($hpCandidate) ? $hpCandidate : null;
        }
    @endphp
    {{-- Iconos de categoria disponibles para la cabecera (megamenu). --}}
    @include('tienda::storefront.partials.iconos-categoria')
    @include($hpHeaderView ?? \App\Modules\Tienda\Support\StorefrontLayoutPacks::view($settings, 'headers') ?? 'tienda::storefront.partials.headers.classic')
    @if($hpKey)
        @includeIf('tienda::storefront.partials.nav.preset-modules')
    @endif
    ...
    <main id="storefront-main" x-show="!cartPageOpen && !checkoutOpen">
        ...
        @php
            ...
            $promoCards = \App\Modules\Tienda\Models\Promotion::where('project_id', $project->id)->vigentes()
        ...
        <section class="promo-cards-section" data-store-native-section="promo_cards" style="order:{{ $sectionOrder('promo_cards') }}" aria-label="Promociones del mes">
            <div class="container">
                <div class="home-section-head">
                    ...
                    <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project) }}/promociones" class="home-see-all">Ver todas <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"/></svg></a>
                </div>
                @php
        ...
        <section class="promo-section" aria-label="Promociones" data-store-native-section="promotions" style="order:{{ $sectionOrder('promotions') }}"
            ...
            <div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'promotions'])
        ...
        <section class="home-cats style-{{ $featuredCatsStyle }} shape-{{ $featuredCatsShape }} {{ ($settings['featured_categories_cover'] ?? '0') === '1' ? 'cover-cards' : '' }} {{ $featuredCatsMobileCarousel ? 'mobile-carousel' : '' }}" data-store-native-section="categories" style="order:{{ $sectionOrder('categories') }};--cats-cols:{{ max(1, min($featuredCatsColumns, $featuredCategoryItems->count())) }}">
            <div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'categories'])
                ...
                <div class="hc-tiles-wrap{{ $tilesPromo ? ' has-promo' : '' }}">
                    <div class="hc-tiles">
                        ...
                        @endphp
                        <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::categoryUrl($project, $cat->id) }}" class="hc-tile">
                    ...
                    @if($tilesPromo)
                    <a class="hc-promo-card" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project) }}/promociones" @if($tilesPromoImg) style="--promo-img:url('{{ $tilesPromoImg }}')" @endif>
...
@unless($introActive('categories'))                <div class="home-section-head"><div class="home-section-copy"><h2>{{ $featuredCatsTitle }}<span class="hc-dot">.</span></h2></div></div>@endunless
        ...
        <section class="trust-section style-{{ $trustSectionStyle }} {{ $trustMobileCarousel ? 'mobile-carousel' : '' }}" aria-label="Beneficios" data-store-native-section="benefits" style="order:{{ $sectionOrder('benefits') }}">
            <div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'benefits'])
@unless($introActive('benefits'))
        ...
        <section class="special-home-section flash-sale-section style-{{ $flashStyle }}" data-store-native-section="flash_sale" style="order:{{ $sectionOrder('flash_sale') }};--flash-bg:{{ $flashBg }}" @if($flashEndsIso) x-data="{deadline:new Date(@js($flashEndsIso)).getTime(),now:Date.now(),timer:null,start(){this.timer=setInterval(()=>this.now=Date.now(),1000)},stop(){if(this.timer)clearInterval(this.timer)},pad(v){return String(Math.max(0,v)).padStart(2,'0')},get distance(){return Math.max(0,this.deadline-this.now)},get days(){return Math.floor(this.distance/86400000)},get hours(){return Math.floor((this.distance%86400000)/3600000)},get minutes(){return Math.floor((this.distance%3600000)/60000)},get seconds(){return Math.floor((this.distance%60000)/1000)}}" x-init="start()" @endif>
            <div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'flash_sale'])
                @if($flashStyle === 'band')
                ...
                <div class="special-product-grid flash-product-grid">
                    ...
                    @include('tienda::public.templates.partials.computienda-card', ['p' => $product])
                    @endforeach
                </div>
                @endif
            </div>
        ...
        <section class="special-home-section discount-section" data-store-native-section="discount_products" style="order:{{ $sectionOrder('discount_products') }}">
            <div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'discount_products'])
@unless($introActive('discount_products'))                <div class="special-section-head">
        ...
        @php
            ...
            $pfCatId = \App\Modules\Tienda\Support\StorefrontNavigation::currentCategoryId();
        ...
        <section class="pf-section" data-store-native-section="featured_products" style="order:{{ $sectionOrder('featured_products') }}"><div class="container">
                @include('tienda::public.templates.partials.computienda-intro', ['introKey' => 'featured_products'])
        ...
        @include('tienda::public.templates.partials.computienda-home-extras')
        @endif
        ...
        <section class="catalog" id="tienda-catalogo" data-store-native-section="catalog" style="order:{{ $sectionOrder('catalog') }}"><div class="container">
            ...
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project) }}/marcas">Marcas</a><span>/</span><strong>{{ $activeBrand->label }}</strong></nav>
            @else
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span><strong x-text="activeFilterLabel">Todos los productos</strong></nav>
            @endif
                ...
                <div class="cat-banner-inner">
                    <div class="cat-banner-copy">
                        ...
                        @php
                            ...
                            $sBase = \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project);
                        @endphp
                        ...
                        <div class="buscar-head" x-data="{ t: 'todos' }">
                            ...
                            <div class="buscar-grupo" x-show="t==='todos'||t==='categorias'">
                                ...
                                @foreach($sCat as $c)
                                <a class="buscar-chip" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::categoryUrl($project, $c->id) }}"><b>{{ $c->name }}</b><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"/></svg></a>
                                @endforeach
                            </div>
                            @endif
        ...
        @php
            $cpBase = \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project); $cpPdf = $cpBase.'/catalogo.pdf';
        ...
        <section class="store-page store-page--catalogo{{ $cpBanner ? ' has-banner' : '' }}" data-store-native-section="custom_page">
            ...
            <div class="cp-banner" @if($cpBannerImg) style="--cp-img:url('{{ $cpBannerImg }}')" @endif>
                <div class="container cp-banner-in">
                    <div class="cp-banner-copy">
                        <nav class="cp-crumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>›</span><strong>Catálogo PDF</strong></nav>
            ...
            <div class="container">
                @unless($cpBanner)
                <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span><strong>Catálogo PDF</strong></nav>
        ...
        <section class="store-page store-page--promociones" data-store-native-section="custom_page">
            <div class="container">
                <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span><strong>Promociones</strong></nav>
                ...
                <div class="catalog-empty">
                    ...
                    <a class="button button-primary" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project) }}/tienda">Ver todos los productos</a>
                </div>
                @else
                <div class="promo-cards-grid">
                    @foreach($promoCards as $pr)
        ...
        @php
            ...
            $baseTienda = \App\Modules\Tienda\Support\StorefrontNavigation::publicUrl($project);
        @endphp
        <section class="store-page store-page--marcas" data-store-native-section="custom_page">
            <div class="container">
                <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span><strong>Marcas</strong></nav>
        ...
        <section class="catalog" data-store-native-section="product" style="order:1"
            ...
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación" style="margin-top:22px">
                <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::homeUrl($project) }}">Inicio</a><span>/</span>
                <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}">Tienda</a><span>/</span>
                @if($sp->category)<a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}?category={{ $sp->category_id }}">{{ $sp->category->name }}</a><span>/</span>@endif
            ...
            <div class="pdp-wrap{{ $pdpB2b ? ' is-b2b' : '' }}">
                ...
                {{-- Info --}}
                <div>
                    @if($sp->category)<a class="pdp-cat" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}?category={{ $sp->category_id }}">{{ $sp->category->name }}</a>@endif
                    @if($sp->marca?->label)
        ...
        <section class="store-page{{ $storeView === 'nosotros' ? ' store-page--nosotros' : '' }}" data-store-native-section="custom_page" style="order:{{ $sectionOrder('custom_page') }}"><div class="container">
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type'=>'home'])) }}">Inicio</a><span>/</span><strong>{{ $storePage?->title ?? ($storeView === 'nosotros' ? 'Nosotros' : 'Contacto') }}</strong></nav>
        ...
        <section class="store-page"><div class="container legal-container">
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type'=>'home'])) }}">Inicio</a><span>/</span><strong>{{ $storePage->title }}</strong></nav>
        ...
        <section class="store-page"><div class="container legal-container">
            <nav class="catalog-breadcrumb" aria-label="Ruta de navegación"><a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type'=>'home'])) }}">Inicio</a><span>/</span><strong>Libro de Reclamaciones</strong></nav>
            <div class="lr-head">
    ...
    @php
        ...
        $shopBaseFooter = \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'shop']));
        ...
        $aboutActiva = \App\Modules\Tienda\Models\StorePage::where('project_id', $project->id)
            ->where('key', 'nosotros')->value('is_enabled');
        $aboutUrl   = ($aboutActiva === null || (int) $aboutActiva === 1)
            ? \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'about']))
            : null;
        $contactUrl = \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, new \App\Modules\Tienda\Models\StoreMenuItem(['destination_type' => 'contact']));
    @endphp
...
@endif
    ...
    @include(\App\Modules\Tienda\Support\StorefrontLayoutPacks::view($settings, 'footers') ?? 'tienda::storefront.partials.footers.classic')
    </div>
    ...
    <div class="mobile-nav-layer" x-show="mobileNav" x-cloak @keydown.escape.window="mobileNav=false" role="dialog" aria-modal="true" aria-label="Menú">
        ...
        <aside class="mobile-nav-panel">
            ...
            <div class="mobile-nav-profiles">
                <a class="profile-chip {{ empty($activeProfile) ? 'is-active' : '' }}" href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::shopUrl($project) }}">Todo</a>
                @foreach($catalogProfiles as $cp)
                    <a class="profile-chip {{ (!empty($activeProfile) && $activeProfile->id === $cp->id) ? 'is-active' : '' }}"
                       ...
                       href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, $cp->slug) }}"><span class="profile-dot" aria-hidden="true"></span>{{ $cp->menu_label ?: $cp->name }}</a>
                @endforeach
            </div>
            @endif
            <nav class="mobile-nav-links">
                @if($menuRoots->count())
                    @foreach($menuRoots as $mItem)
                        ...
                        @else
                            <a href="{{ \App\Modules\Tienda\Support\StorefrontNavigation::resolveUrl($project, $mItem) }}" @click="mobileNav=false" @if($mItem->target === '_blank') target="_blank" rel="noopener" @endif>{{ $mItem->label }}</a>
                        @endif
                    @endforeach
                @endif
                    ...
                    @php
                        ...
                        foreach (($catalogProfiles ?? collect()) as $mp) {
                            ...
                            $mnavAccesos[] = ['t' => $mp->menu_label ?: $mp->name,
                                              'u' => \App\Modules\Tienda\Support\StorefrontNavigation::profileUrl($project, $mp->slug),
                            ...
                            @if($cat->children->count())
                                <div class="mnav-grp" x-data="{ abierto:false }">
                                    <button type="button" class="mnav-cat" @click="abierto=!abierto" :class="abierto && 'is-open'" :aria-expanded="abierto">
                                        <span class="mnav-ico" aria-hidden="true">{!! \App\Modules\Tienda\Support\CategoryIcons::svg($cat->name, $settings['caticon_'.$cat->id] ?? null) !!}</span>
    ...
    @include(\App\Modules\Tienda\Support\StorefrontLayoutPacks::view($settings, 'carts') ?? 'tienda::storefront.partials.carts.classic')
