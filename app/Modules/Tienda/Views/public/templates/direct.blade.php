...
@include('tienda::public.partials.variant-engine')
...
{{-- ─── HEADER ─────────────────────────────────────────────────────────────── --}}
@include('tienda::public.partials.header')
  ...
  <div class="flex-1 min-w-0">
    ...
    <div x-show="filterCat==='' || filterCat==='{{ $cat->id }}' || filterParent==='{{ $cat->id }}' || [{{ $cat->children->pluck('id')->map(fn($id)=>"'".$id."'")->join(',') }}].includes(filterCat)">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 mb-8" data-products-grid>
        ...
        @include('tienda::public.partials.storefront-card', ['item' => $catalogo->toCard($p, $project->slug), 'catId' => $cat->id, 'parentId' => null, 'idx' => $idx])
        @endforeach

        {{-- Productos de subcategorías --}}
        @foreach($cat->children as $sub)
        @foreach($sub->products as $idx => $p)
        @include('tienda::public.partials.storefront-card', ['item' => $catalogo->toCard($p, $project->slug), 'catId' => $sub->id, 'parentId' => $cat->id, 'idx' => $idx])
        @endforeach
        @endforeach
      </div>
    </div>
...
{{-- ─── FOOTER ─────────────────────────────────────────────────────────────── --}}
@include('tienda::public.partials.footer', ['footerBg'=>$footerBg,'footerText'=>$footerText,'footerLogoH'=>$footerLogoH,'logoSrc'=>$logoUrl])
