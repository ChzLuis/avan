...
<section class="bxb-stage">
    ...
    <div class="bxb-card" x-data='hpPresetPicker(@json(\App\Modules\Tienda\Support\HeaderPresets::forBuilder()))'>
    ...
    <div class="bxb-card" x-show="!(settings.header_preset||'')">
        ...
        <label class="bxb-field">Diseño de la cabecera
            <select :value="settings.header_layout||'classic'" @change="setSetting('header_layout',$event.target.value)">
                @foreach(\App\Modules\Tienda\Support\StorefrontLayoutPacks::options('headers') as $vKey => $vLabel)
                <option value="{{ $vKey }}">{{ $vLabel }}</option>
                @endforeach
            </select>
            {{-- Qué se va a ver con cada diseño: el nombre solo no alcanza para elegir. --}}
            <span class="bxb-note" x-text="(@js(\App\Modules\Tienda\Support\StorefrontLayoutPacks::DESCRIPCIONES['headers']))[settings.header_layout||'classic'] || ''"></span>
        </label>
