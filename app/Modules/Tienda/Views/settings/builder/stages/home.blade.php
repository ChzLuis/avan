        ...
        <div>
            ...
            <div class="bxb-card">
                ...
                <div class="bxb-tpl-grid" role="radiogroup" aria-label="Diseño de la página de inicio">
                    @foreach(\App\Modules\Tienda\Storefront\HomePresets::todos() as $hpKey => $hp)
                        <button type="button" class="bxb-tpl bxb-hp"
                            :class="(settings.home_template||'{{ \App\Modules\Tienda\Storefront\HomePresets::POR_DEFECTO }}')==='{{ $hpKey }}'&&'is-active'"
                            :aria-checked="((settings.home_template||'{{ \App\Modules\Tienda\Storefront\HomePresets::POR_DEFECTO }}')==='{{ $hpKey }}').toString()"
                            role="radio"
                ...
                <p class="bxb-note" x-text="{{ Js::from(collect(\App\Modules\Tienda\Storefront\HomePresets::todos())->map->resumen) }}[settings.home_template||'{{ \App\Modules\Tienda\Storefront\HomePresets::POR_DEFECTO }}']"></p>
                ...
                <div class="bxb-tpl-grid" role="radiogroup" aria-label="Variante de portada">
                    @foreach(['a' => 'Variante A', 'b' => 'Variante B'] as $hvKey => $hvLabel)
                        <button type="button" class="bxb-tpl bxb-hp"
                            ...
                            <small x-text="{{ Js::from(collect(\App\Modules\Tienda\Storefront\HomePresets::todos())->map->{'hero'.strtoupper($hvKey)}) }}[settings.home_template||'{{ \App\Modules\Tienda\Storefront\HomePresets::POR_DEFECTO }}']"></small>
                        </button>
                    @endforeach
                </div>
            </div>
