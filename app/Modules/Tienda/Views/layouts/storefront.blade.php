@php
    $storefrontTheme = $storefrontTheme ?? \App\Modules\Tienda\Support\StorefrontTheme::resolve($settings ?? []);
@endphp
<!doctype html>
<html lang="es">
<head>
    ...
    @stack('styles')
    @include('tienda::components.storefront.theme-styles', ['theme' => $storefrontTheme])
</head>
<body class="storefront-theme storefront-theme-{{ $storefrontTheme['key'] }} storefront-family-{{ $storefrontTheme['family'] }} storefront-card-{{ $storefrontTheme['card_style'] }}" data-store-template="{{ $storefrontTheme['key'] }}" data-store-theme-family="{{ $storefrontTheme['family'] }}">
    ...
    @include('tienda::components.storefront.header')
    <main id="contenido-principal" class="store-main">@yield('content')</main>
    @include('tienda::components.storefront.footer')
    @if(isset($popup) && $popup)@include('tienda::components.public-popup', ['popup'=>$popup])@endif
    @stack('scripts')
</body>
</html>
