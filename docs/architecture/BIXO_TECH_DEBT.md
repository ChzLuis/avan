...
| TD-023 | MEDIUM | Workspace | **Unificacion visual fase 2** | tras variantes | **CERRADA 2026-08-30**: `AppLayout` delega en el shell comercial cuando hay negocio activo, asi que las 20+ vistas con `<x-app-layout>` (Productos y Constructor incluidos) heredan el menu unico SIN tocarlas — no habia que esperar a variantes. El modal global `__confirm` se extrajo a `partials/confirm-global` para los dos shells: vivia dentro del shell del panel y las pantallas migradas se quedaban sin el (eliminar producto/imagen/categoria y descartar borrador reventaban en silencio). `ShellUnicoWorkspaceTest` 10/10 |
