...
| TD-025 | MEDIUM | Workspace | Aplicacion fina de capacidades | proxima sesion | **CERRADA 2026-08-30**: `saveDraftSettings` (la puerta real del Constructor) veta las claves restringidas y devuelve `omitidas`; cap_seo_avanzado cubre analitica/pixeles/robots/schema/verificaciones y cap_builder_avanzado el cambio de motor. `CapacidadesRestringidasTest` 7/7, incluido el caso del cliente que intenta escribirlas y NADA se guarda |
| TD-026 | MEDIUM | Repo | **Deriva bidireccional en `SettingsController`**: ARIN tiene 20 campos de QR y el ProcesadorImagenes que local no trae; local tiene decisiones de arquitectura que ARIN no (no aceptar telefono/redes/SEO por esa puerta, retiro de superficies legacy). Adoptar ARIN a ciegas ROMPE el asistente de presets (comprobado con test). Reconciliar con calma, metodo por metodo, y cubrir con tests antes de desplegar | proxima sesion | ABIERTA |
