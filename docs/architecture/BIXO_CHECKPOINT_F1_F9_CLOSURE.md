...
| H8 client_id | **PARCIAL** (parte estable cerrada) | Panel (pedidos/cotizaciones) ya enlazaba; comprobantes ahora enlazan por teléfono/correo al cliente existente (`InvoiceClientLinkTest` 2/2, en ARIN). Falta: checkout web y bot (PublicController/BotWebhookController, feature ajena en vuelo); guías (Client sin columna de documento). |
